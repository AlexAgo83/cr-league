...
- Normalize circuit lap counts or replay scaling from measured route distance instead of manual guesswork.
- Make overtakes and gaps readable through scripted arcade beats that match the final result.
...
- In: lap-count or replay-scaling normalization that keeps city circuits distinct while making race duration feel comparable.
- In: a deterministic replay staging adapter that converts simulation trace data into presentation beats for starts, gaps, overtakes, weather changes, and finishes.
...
- Circuit normalization starts from measured route display distance instead of subjective lap-count tweaking.
...
- Players can watch a pass happen through approach, overlap or offset, swap, and settle phases instead of a sudden ranking jump.
