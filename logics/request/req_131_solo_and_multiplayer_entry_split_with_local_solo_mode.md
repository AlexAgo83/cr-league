...
# Needs
- Replace the current top-level league setup choice with a clear Solo / Multiplayer mode choice after profile setup and whenever no active league is loaded.
...
- Reuse the existing LeagueState shape, game screens, race preparation loop, replay/report surfaces, cards, garage, championship, and shared simulation helpers as much as possible instead of creating a separate game UI.
...
- Profile setup currently requires API-backed profile creation/recovery before league setup. For a true no-network solo path, decide whether Solo bypasses profile setup entirely or whether the top-level mode selector appears before profile setup. The preferred direction is: root splash -> Solo / Multiplayer; Solo can start local without profile; Multiplayer continues through the existing profile and league setup gates.
...
- AC3: Choosing Solo starts or resumes a local solo league without requiring API availability, profile creation, recovery code, invite code, or multiplayer claim.
...
- AC5: Solo progress survives reload via local storage and is isolated from multiplayer profile/session/claim storage. Forgetting or logging out of a multiplayer profile does not corrupt solo progress unless a dedicated solo reset command is used.
