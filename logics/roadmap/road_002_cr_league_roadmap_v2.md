...
> Semantic edit: 2026-07-22 third code-review audit sweep: scaffolded simulation fidelity and replay performance (`req_087`) as 0.4.10 — qualifying was ignoring circuit traits entirely, CircuitMap re-ran an O(n^2) scene analysis every replay frame, the recap mislabeled heavy rain, and `/simulation/preview` could return NaN; sequenced after `req_085`/`req_086` and before the card-economy unhold.
> Semantic edit: 2026-07-22 verified `req_085`/`req_086`/`req_087` all landed in code (HEAD 611bd57, tests green, 0 npm-audit vulns) and marked 0.4.9/0.4.10 Shipped; fourth audit sweep of the not-yet-reviewed surfaces scaffolded post-remediation hardening (`req_088`) as 0.4.11 — a residual submit-vs-sell card race, client security/privacy (unsanitized livery, recovery-code leak to a11y/logs, cleartext API fallback, no CSP), a replay accessibility baseline, RaceDecision data-model indexes/FK/constraints, and required-env/admin config validation.
> Semantic edit: 2026-07-22 user-reported replay defect: pitting cars halt at the wrong on-map position because the sim stops the car at a hardcoded 0.5 half-lap (store.ts:724) while the map draws the pit from route geometry; scaffolded pit-stop visual alignment (`req_089`) as 0.4.12 to derive one per-circuit pit-lane progress from geometry, expose it as shared data, and feed it into the sim so cars stop where the pit is drawn.
...
- The first three review-driven chains have shipped and are verified in code (HEAD 611bd57, tests green, 0 npm-audit vulns): `req_085` / `task_086` (pass 6), `req_086` / `task_087` (gameplay/economy integrity), and `req_087` / `task_088` (0.4.10 simulation fidelity). Two chains are now active at the 0.4/0.5 boundary: `req_088` / `task_089` (0.4.11 post-remediation hardening) from the fourth audit sweep, and `req_089` / `task_090` (0.4.12 pit-stop visual alignment) from a user-reported replay defect.
...
- Delivered or active patches:
  ...
  - 0.4.9 — Repo review remediation pass 6: close JSON-column race locks, restore simulation finishing-order fidelity, guard destructive/admin authority changes, derive plan badges from shared stat deltas, and delete the surfaced over-engineering. Shipped through `req_085` / `task_086` (verified landed 2026-07-22, HEAD 611bd57).
  - 0.4.10 — Simulation fidelity and replay performance: wire the circuit's numeric traits into qualifying (times were ignoring the track), memoize the CircuitMap scene/route analysis so it stops recomputing an O(n^2) scan every replay frame, fix the pop-timer leak, correct the recap heavy-rain label and podium/loss verdict, make ambient cars move forward-only, translate the focus-driver aria-label, validate `/simulation/preview` traits against NaN, and de-bias the per-season circuit shuffle. Shipped through `req_087` / `task_088` (verified landed 2026-07-22).
  - 0.4.11 — Post-remediation hardening: close the residual submit-vs-sell card race (re-validate card ownership inside the locked tx), harden the client (sanitize livery colors, stop leaking the recovery code to a11y/logs, refuse cleartext API bases, add a CSP/frame-ancestors baseline), add a replay accessibility baseline (prefers-reduced-motion, modal focus trap, listbox keyboard nav), tighten the RaceDecision data model (teamId index, rivalTeamId FK, enum constraints, concurrent index migration), and fail fast on missing prod env plus declare the admin envs. Active through `req_088` / `task_089`.
  - 0.4.12 — Pit-stop visual alignment: pitting cars halt at the wrong on-map spot because the sim uses a hardcoded 0.5 half-lap position while the map draws the pit from route geometry; derive one per-circuit from-start pit-lane progress from geometry, expose it as shared circuit data (generated + audited so it cannot drift), and feed it into the simulation so cars stop exactly where the pit garage is drawn. Active through `req_089` / `task_090`.
- Exit signal:
...
- `task_088` / `req_087` (0.4.10) has shipped: qualifying now responds to the track, replay render cost is bounded, and recap/overlay accuracy and public-input robustness are fixed.
- Then finish `task_089` / `req_088` (0.4.11 post-remediation hardening): close the residual submit-vs-sell card race first (data integrity), then the client security/privacy, accessibility, data-model, and config items on the same chain.
- `task_090` / `req_089` (0.4.12 pit-stop visual alignment) can run in parallel or after: it is a self-contained replay-fidelity fix (derive one per-circuit pit-lane progress, feed it into the sim) with no overlap with the hardening chain.
- After the hardening chain is green, run the fresh AI-playtest/balance baseline before unholding `req_081` card economy rebalance.
...
- JSON-column inventory/decision writes are now known risk areas; keep new card/team mutations on the row-lock/runWrite pattern from `req_085`. Cross-operation locking still bites: `req_088` closes a submit-vs-sell race where `submitDecision` locks the GP row while `sellCard` locks the team row — operations that touch the same card must serialize on the same row, not just each be transactional.
...
- `task_088` / `req_087` (0.4.10) is shipped and verified in code.
- Next: `task_089` / `req_088` (0.4.11 post-remediation hardening). Its submit-vs-sell card race is High (data integrity); the client-security, accessibility, data-model, and config items ride the same chain.
- In parallel: `task_090` / `req_089` (0.4.12 pit-stop visual alignment), a self-contained replay-fidelity fix from a user-reported defect (cars halt at the wrong on-map pit position).
- After those chains are Done, rerun playtest/balance evidence and decide whether to unhold `req_081` card economy rebalance.
