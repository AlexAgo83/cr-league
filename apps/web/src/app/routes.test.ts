...
describe("app routes", () => {
  it("maps stable navigation paths to app state", () => {
    ...
    expect(parseAppRoute("/plan/report")).toEqual({ view: "plan", planSubscreen: "report", directiveStep: "approach", championshipTab: "standings", garagePanel: "inventory" });
  ...
  it("builds canonical paths from app state", () => {
    ...
    expect(pathForAppRoute({ view: "plan", planSubscreen: "report", directiveStep: "approach", championshipTab: "standings", garagePanel: "inventory" })).toBe("/plan/report");
