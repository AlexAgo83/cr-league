...
- Held and coupled — card economy (`req_081`) + stat differentiation: the three circuit traits share one formula shape and the trait interaction is far weaker than the flat pace terms (audit cause A), so circuit identity barely changes the best answer. Owner decision 2026-07-21: do NOT start `req_081` yet. Rebalancing cards against the current flat stat model would be invalidated once stats diverge, so the two are one coupled depth pass. Unblock only when the stat-model direction is decided; then co-design the card rebalance and a stat-differentiation request together against a fresh AI-playtest baseline so per-circuit strategies actually diverge.
