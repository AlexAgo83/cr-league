...
- Repo review remediation and beta support hardening (`req_058`, `req_069`) are already Done and count as delivered 0.4 foundation, not future patch slots.
- Eight implementation chains are open and ready to dev: `req_068` (non-winning success feedback), `req_070` (web view code-splitting), `req_071` (stylesheet modularization), `req_072` (WebP artwork delivery), `req_073` (lazy artwork loading), `req_074` (circuit route loading audit), `req_075` (GameApp admin-panel hook extraction), and `req_076` (league-state freshness on return).
...
- Patch history and deferred slots:
  ...
  - 0.3.15 — Non-winning success feedback: deferred from the 0.3 line; tracked in the post-0.3 playtest-loop carry-over queue below with `req_068` / `task_069` ready to dev.
  ...
  - 0.3.17 — League-state freshness on return: deferred from the 0.3 line; tracked in the post-0.3 playtest-loop carry-over queue below. Real-time polling/SSE stays a 0.6 decision.
...
- Non-winning success feedback: `req_068` / `task_069` remain ready to dev and can ship as a gameplay-feedback patch whenever tester comprehension outranks payload/support work.
- League-state freshness on return: `req_076` / `task_077` are ready to dev as the small visibility-refetch implementation; keep polling/SSE deferred to 0.6 unless beta evidence proves realtime is necessary.
...
- Already-delivered foundation: CI/release contract, hosted release health checks, email-backed profile recovery, repo review remediation pass 5, and beta support hardening are complete through earlier request chains (`req_036`, `req_061`, `req_058`, `req_069`).
- Planned patches:
  ...
  - 0.4.7 — League-state freshness on return: silently refetch the active league when the player returns to a visible tab, without polling or realtime infrastructure. (`req_076`, `task_077`, ready to dev)
  - 0.4.8 — Race weather and card stat readability: move resolved weather phase detail behind a circuit info modal, wrap card stat badges cleanly, and expose stat explanations on badge hover/focus. (`req_083`, `task_084`, ready to dev)
- Exit signal:
...
- Status: Opening; the card-economy evidence gate is met (audit_trial_2107 AI playtest). Opponent config comparison (`req_082`) proceeds now; card economy rebalance (`req_081`) is scaffolded but HELD (see coupling below).
...
- Prerequisite depth pass — circuit stat differentiation and bot strategy (`req_084`): force Grip, Attack, and Endurance to create distinct setup pressure, and make bots vary pit strategy deterministically by circuit identity before card tuning resumes.
- Held and coupled — card economy (`req_081`) after `req_084`: the three circuit traits share one formula shape and the trait interaction is far weaker than the flat pace terms (audit cause A), so circuit identity barely changes the best answer. Owner decision 2026-07-21: do NOT start `req_081` yet. Rebalancing cards against the current flat stat model would be invalidated once stats diverge. Unblock only after `req_084` lands with a fresh AI-playtest baseline; then rebalance cards against the differentiated stat model.
...
- Run `req_075` admin-panel extraction as a maintainability follow-up to the already-delivered admin/support hardening.
- Keep `req_076` available as a small 0.4 responsiveness patch before invited multi-player testing, or pull it forward as a hotfix if stale tab state appears in hosted use.
...
- Keep `task_077` / `req_076` ready as the next small responsiveness patch before real multi-player testing.
