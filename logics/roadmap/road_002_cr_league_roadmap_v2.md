...
- Planned patches:
  ...
  - 0.3.22 — Post-playtest clarity and release hardening: result verdicts, first-GP action clarity, replay suspense, lap-scale labels, mobile modal polish, branded splash, compatible Dependabot updates, and release health-gate fixes. Shipped in `changelogs/CHANGELOGS_0_3_22.md`. Follow-up UI copy now names the main cockpit screen `Stand` instead of `Course`/`Race`, keeps profile validation errors inline, and shortens emailed recovery codes to a readable 12-character format.
- Exit signal:
