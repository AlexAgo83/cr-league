...
- Planned patches:
  ...
  - 0.3.13 — Plan risk/readability layer: show a compact `safe / risky / high-upside` read before sending the plan, including where the setup is strong, where it can fail, and which finishing band it is trying to optimize. (`req_067`, `task_068`, ready to dev)
  ...
  - 0.3.15 — Non-winning success feedback: make defensive, economy, and weather plans visibly rewarding when they save risk, preserve a target position, amortize bad weather, or turn credits into future options even without a win. (`req_068`, `task_069`, ready to dev)
  ...
  - 0.3.22 — Post-playtest clarity and release hardening: result verdicts, first-GP action clarity, replay suspense, lap-scale labels, mobile modal polish, branded splash, compatible Dependabot updates, and release health-gate fixes. Shipped in `changelogs/CHANGELOGS_0_3_22.md`. Follow-up UI copy now names the main cockpit screen `Stand` instead of `Course`/`Race`, keeps profile and join-league validation errors inline, shortens emailed recovery codes to a readable 12-character format, makes mobile modals full-screen with bottom-pinned actions, keeps the launch-GP starting grid compact on phones, lets plan card choices collapse to one column on mobile, uses stable SVG replay controls, uses splash title assets in the topbar brand, and reuses the styled current-plan summary in send-plan and chrono confirmations.
- Exit signal:
