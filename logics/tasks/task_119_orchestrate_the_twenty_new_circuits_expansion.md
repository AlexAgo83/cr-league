...
- 2026-07-26: adjacent UI polish while keeping the active wave commit-ready: sliced `logics/external/board_icones.png` plus the follow-up board 2/3 sheets into transparent CRL board assets, wired grip/overtaking/energy/weather/strategy through `VisualIcon`, surfaced larger/dedicated board icons in directive/chrono/GP/map/navigation commands with centered text/icon alignment, added card-context board icons before Garage inventory/shop card names and tabs, and moved Plan card info into a bottom-right `(i)` modal trigger while the cell shows the fit label.
