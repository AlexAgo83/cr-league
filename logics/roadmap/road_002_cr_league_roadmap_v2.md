...
- Three implementation chains are now active at the 0.4/0.5 boundary: `req_085` / `task_086` for repo review remediation pass 6, `req_086` / `task_087` for gameplay/economy integrity, and `req_087` / `task_088` (0.4.10) for simulation fidelity and replay performance.
...
- Delivered or active patches:
  ...
  - 0.4.10 — Simulation fidelity and replay performance: wire the circuit's numeric traits into qualifying (times currently ignore the track), memoize the CircuitMap scene/route analysis so it stops recomputing an O(n^2) scan every replay frame, fix the pop-timer leak, correct the recap heavy-rain label and podium/loss verdict, make ambient cars move forward-only, translate the focus-driver aria-label, validate `/simulation/preview` traits against NaN, and de-bias the per-season circuit shuffle. Active through `req_087` / `task_088`.
- Exit signal:
...
- Then finish `task_088` / `req_087` (0.4.10): it restores qualifying's response to the track, bounds replay render cost, and corrects recap/overlay accuracy and public-input robustness; sequence it after the two integrity chains and before the card-economy unhold, since accurate qualifying and monotonic payouts are both baseline for a fair balance pass.
...
- Then `task_088` / `req_087` (0.4.10 simulation fidelity and replay performance). Its qualifying-trait item is High because qualifying currently ignores the circuit; the replay-perf, recap, and robustness items ride the same chain.
