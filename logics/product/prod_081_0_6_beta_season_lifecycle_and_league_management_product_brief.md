...
> Semantic edit: 2026-07-28 synced delivered season economy rule: rollover resets points only while preserving credits and garage cards.
...
- Season economy continuity: preserve players, palmares, archived season stats, cosmetic/team identity, credits, and garage cards across season rollover; reset championship points only. Reopen caps or anti-snowball rules only if balance evidence proves carry-over is breaking repeated seasons.
...
- Commissioner UX: expose creator controls inside the existing league context as `Direction de course`, via a creator-only tab or button rather than a hidden standalone route. Design it as a CR League control room with clear race-state hierarchy, readiness lanes, compact action groups, and existing visual components. League-management settings include cadence, preparation deadline, and a validated league-name rename. Avoid a generic SaaS table-first admin page. On mobile, use stacked sections; only use sticky actions if that pattern already exists in the app.
- Shared section switching: use the `Plan / Chrono / GP` board-icon switch model for top-level in-view switches such as Championship record tabs and Garage panels. Keep these controls at the top of their view, preserve the existing routes/state keys, and avoid burying navigation inside panel headers.
