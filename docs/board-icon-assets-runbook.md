...
The replay reaction emotes follow the same pipeline but land in their own directory,
`apps/web/public/assets/crl/emotes`, and are resolved by id from
`packages/shared/src/domain/raceEmotes.ts`.

- Prompt: `logics/design/race-replay-driver-emotes/prompt.md`, generated by
  `logics-manager design prompt --kind icon-sheet --cell-size 256x256 --cells "..."`.
- The delivered sheet came back with an opaque gradient background, so the `rembg` pass below is
  required rather than optional.
- One cell bled into the row beneath it. When that happens, do not tune the whole-grid script:
  crop that cell's real object bounds manually, as the "Manual Recrop" section describes. A
  bounding box reaching within ~15 px of the cell edge is the tell.
- `apps/web/src/features/raceEmoteAssets.test.ts` fails if a declared emote id has no committed
  asset, so adding an id to `EmoteId` without shipping artwork is caught by the suite.
