...
function createQualifyingResult(teamId: string, teamName: string, seed: string, decision: RaceDecision, lapTimes: number[], finishWeather: Weather, trackLengthMeters: number): RaceResult {
  ...
  return {
    ...
    events,
    replayTrace: createQualifyingReplayTrace(teamId, lapTimes.length, visualTime, trackLengthMeters),
    consumedCards: [],
    report: {
...
function createQualifyingReplayTrace(teamId: string, laps: number, visualTime: number, trackLengthMeters: number) {
  ...
  return Array.from({ length: laps * stepsPerLap + 1 }, (_, index) => {
    ...
    return {
      segment: RACE_SEGMENTS[Math.min(RACE_SEGMENTS.length - 1, Math.floor(progress * RACE_SEGMENTS.length))] ?? "start",
